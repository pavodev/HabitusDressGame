...
export default function DressGuesserSurvival() {
    ...
    <main className="h-screen overflow-hidden pt-16 p-4 container mx-auto flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
        <div className="rounded-xl border border-gray-200 p-5 bg-white shadow-sm flex flex-col">
          <h2 className="text-lg font-semibold text-gray-900 mb-2">Vite</h2>
          <div className="flex items-center justify-center gap-2">
            {[0, 1, 2].map((i) => (
              <div key={i} className={`w-8 h-8 flex items-center justify-center rounded-full border ${i < lives ? 'bg-red-500 text-white border-red-600' : 'bg-gray-100 text-gray-400 border-gray-300'}`}>
                ❤
              </div>
            ))}
          </div>
          <p className="text-sm text-gray-500 mt-3 text-center">{lives} / 3 vite rimaste</p>
        </div>
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-3 flex gap-3 justify-center">
            <a role="button" className="after:content-['✓']" onClick={confirmPick} aria-disabled={lives <= 0} style={{ pointerEvents: lives <= 0 ? 'none' : 'auto', opacity: lives <= 0 ? 0.6 : 1 }}>Conferma scelta</a>
      ...
      {showLoseModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 mx-4 max-w-md w-full shadow-2xl text-center">
            <div className="text-6xl mb-4">💀</div>
            <h2 className="text-2xl font-bold mb-2 text-gray-900">Hai esaurito le vite</h2>
            <p className="text-gray-600 mb-6">La risposta corretta era: <span className="font-medium">{target.name}</span></p>
            <div className="flex gap-3">
              <button className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700" onClick={restartGame}>Ricomincia</button>
            </div>
          </div>
        </div>
      )}
